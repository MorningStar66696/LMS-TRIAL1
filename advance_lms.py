...
def main_page():
    ...
    for col in df_display.columns:
        ...
        if "date" in col.lower():
            # Safely handle empty date columns
            if filtered_df[col].dropna().empty:
                continue  # skip this date filter if all values are empty
            min_date = filtered_df[col].min()
            max_date = filtered_df[col].max()
            default_range = date_ranges.get(col, [min_date, max_date])
            try:
                ...
                st.warning(f"⚠️ Could not create date filter for column '{col}': {e}")
        else:
