...
for key in ['filtered_df', 'selected_values', 'date_ranges', 'page_number', 'select_all_state']:
...
def main_page():
    ...
    for col in df.columns:
        ...
        if "date" in col.lower():
            ...
            # Multi-select filter with search and select-all
            ...
            select_all = st.sidebar.checkbox(f"Select All {col}", key=f"select_all_{col}", value=select_all_state.get(col, False))
    ...
    col1.download_button("Download Filtered Data", filtered_df.to_csv(index=False).encode('utf-8'), "filtered_data.csv")
    col2.download_button("Download All Data", df.to_csv(index=False).encode('utf-8'), "all_data.csv")
